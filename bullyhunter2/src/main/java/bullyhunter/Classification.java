...
public class Classification {
    ...
    public static double classifyTweet(String msg) {
        ...
        double num = classifier.classify(t2v.getIndexSet(), t2v.getValueSet());
        return num;
    ...
    public static boolean checkInput(String text) {
        if (text.length() > 150) {
            System.err.println("(Warning: line longer than 150 characters): "
                    + text);
            return false;
        }
        ...
        if (containKeyword == false) {
            System.err.println("(Warning: line does not contain keywords): "
                    + text);
            return false;
        }
        if (!lowerCase.contains("bull")) {
            System.err
                    .println("(Warning: line does not contain string \"bull\"): "
                            + text);
            return false;
        }
        if (text.contains("RT")) {
            System.err.println("(Warning: line contains \"RT\", retweet?): "
                    + text);
            return false;
        }
        return true;
